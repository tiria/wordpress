...
<div <?php post_class( 'article-content' ); ?>>
	<?php
	$mediaURL = apply_filters( 'wmhook_format_video_mediaURL', trim( get_post_meta( get_the_ID(), '_format_video_embed', true ) ) );

	if ( ! $mediaURL ) {
		$mediaURL = trim( wm_meta_option( 'video-url' ) );
	}
	...
	if ( false !== strpos( $mediaURL, '[video ' ) ) {
		...
		echo do_shortcode( '[video url="' . esc_url( $mediaURL ) . '" poster="' . $imageSrc[0] . '" /]' );

	} else {

		echo do_shortcode( '[box color="red" icon="warning"]' . __( 'Please set the video URL address', 'clifden_domain' ) . '[/box]' );
